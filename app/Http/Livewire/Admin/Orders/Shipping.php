...
class Shipping extends Component
{
    ...
    protected $listeners = [
        'deliverOrders' => 'passedOrder',
        'deletedDeliver' => 'delete',
    ];
    ...
    public function selectedOrders()
    {
        if (count($this->selectedCheckboxes) > 0) {
            foreach ($this->selectedCheckboxes as $model) {
                Order::findOrFail($model)->update(['status' => 4]);
            }
        }
    ...
    public function confirm($id)
    {
        $this->dispatchBrowserEvent('swal:confirm', [
            'type' => 'warning',
            'title' => 'Are you sure want to mark this as delivered order?',
            'text' => 'This action will mark the selected order as delivered order so it will be removed from shipping orders',
            'id' => $id,
        ]);
    }
    public function passedOrder($id)
    {
        $updateStatus = Order::findOrFail($id)->update(['status' => 4]);

        if ($updateStatus) {
            $this->dispatchBrowserEvent('swal:modal', [
                'type' => 'success',
                'title' => 'Order has passed to delivered orders successfully!',
                'text' => 'Find the order in delivered orders list',
            ]);
        }
        //if no pending orders left admin will be directed to confirmed orders
        if (Order::whereStatus($this->orderType)->count() == 0) {
    ...
    public function render()
    {
        ...
        return view('livewire.admin.orders.shipping', compact('orders'));
    }
}
